...
export default {
  ...
  methods: {
    ...
    initCVS() {
      ...
      const [ctx1, ctx2, ctx3, ctx4, ctx5] = this.cvsDocus.map(c => c.getContext('2d'));
      ...
      // fillStyle 设置或返回用于填充绘画的颜色、渐变或模式。
      this.cvss[0].fillStyle = '#000';
      this.cvss[1].fillStyle = this.textColor;
      // 根据屏幕宽度调整大小
      this.resize()
    },
    resize() {
      let box = document.getElementsByClassName('canvas-box')
      // 屏幕宽度小于99999999，textFontSize开始自减，调整字体大小，适应屏幕宽度
      while (this.textWidth > window.innerWidth) {
        this.$emit('reduceFontSize')
        // 在画布上写一段900粗细 textFontSize像素的文本，使用的字体是 "Arial"
        this.cvss[0].font = `900 ${this.textFontSize}px Arial`;
        // measureText 返回包含指定文本宽度的对象。
        this.textWidth = this.cvss[0].measureText(this.textArr[0]).width;
      }
      while (this.textWidth2 > window.innerWidth) {
        this.cvss[1].font = `900 ${this.textFontSize}px Arial`;
        this.textWidth2 = this.cvss[1].measureText(this.textArr[1]).width;
      }
      // this.cvsDocus[0].width = this.textWidth * 1.5;
      this.cvsDocus[0].width = box[0].offsetWidth;
      this.cvsDocus[1].width = box[0].offsetWidth;
      // this.cvsDocus[0].height = this.textFontSize * 1.5;
      this.cvsDocus[0].height = box[0].offsetHeight;
      this.cvsDocus[1].height = box[0].offsetHeight;
      this.cvsDocus[0].textAlign="center";
      this.cvsDocus[1].textAlign="center";
      this.cvss[0].font = `${this.textWeight} ${this.textFontSize}px Arial`;
      this.cvss[1].font = `${this.textWeight} ${this.textFontSize}px Arial`;
      this.cvss[0].fillText(this.textArr[0], (this.cvsDocus[0].width - this.textWidth) / 2, this.textFontSize);
      this.cvss[1].fillText(this.textArr[1], (this.cvsDocus[1].width - this.textWidth) / 2, 50);
      this.getPosition();
    },
    // 给文字像素定位，并且填充
    getPosition() {
      ...
      */
      const imgData = this.cvss[0].getImageData(0, 0, this.cvsDocus[0].width, this.cvsDocus[0].height);
      const imgData2 = this.cvss[1].getImageData(0, 0, this.cvsDocus[1].width, this.cvsDocus[1].height);
      for (let i = 0, max = imgData.data.length; i < max; i += 4) {
        ...
        if (alpha && x % this.fidelity === 0 && y % this.fidelity === 0) {
          this.textTargetsO.push({ x, y });
        }
      }
      for (let i = 0, max = imgData2.data.length; i < max; i += 4) {
        const alpha = imgData2.data[i + 3];
        const x = Math.floor(i / 4) % imgData2.width - 200;
        const y = Math.floor(i / 4 / imgData2.width);
        if (alpha && x % this.fidelity === 0 && y % this.fidelity === 0) {
          this.textTargetsT.push({ x, y });
        }
      }
      // 设置或返回用于填充绘画的颜色、渐变或模式。
      this.cvss[3].fillStyle = '#FFF';
      // 设置或返回用于填充绘画的颜色、渐变或模式。
      this.cvss[4].fillStyle = this.textColor;
      if(this.showShadow) {
        // 设置或返回用于阴影的颜色
        this.cvss[3].shadowColor = this.shadowColor1;
        // 设置或返回用于阴影的模糊级别
        this.cvss[3].shadowBlur = this.shadowBlur1;
        // 设置或返回用于阴影的颜色
        this.cvss[4].shadowColor = this.shadowColor2;
        // 设置或返回用于阴影的模糊级别
        this.cvss[4].shadowBlur = this.shadowBlur2;
      }
    ...
    loop() {
      ...
      // fillRect 绘制"被填充"的矩形。
      this.cvss[2].fillRect(0, 0, this.cvsDocus[2].width, this.cvsDocus[2].height);
      this.cvss[2].fillRect(0, 0, this.cvsDocus[4].width, this.cvsDocus[4].height);
      this.counter += 1;
      ...
      this.shards.forEach((s, i) => {
        ...
        if (s.timer >= s.ttl || s.lightness >= 99) {
          this.cvss[3].fillRect(s.target.x, s.target.y, this.fidelity + 1, this.fidelity + 1);
          this.cvss[4].fillRect(s.target.x, s.target.y, this.fidelity + 1, this.fidelity + 1);
    ...
    getTarget() {
      ...
      if (this.textTargetsT.length > 0) {
        ...
        x += this.cvsDocus[4].width / 2 - this.textWidth / 2;
        y += this.cvsDocus[4].height / 2 - this.textFontSize / 2;
      ...
      if (this.textTargetsO.length > 0) {
        ...
        y += this.cvsDocus[2].height / 2 - this.textFontSize / 2;
        return { x, y };
      }
    ...
    resetCanvas() {
      ...
      this.cvss.forEach((item, index) => {
        item.clearRect(0, 0, this.cvsDocus[index].width, this.cvsDocus[index].height)
        item.restore();
      })
      this.isShow = false
      this.$nextTick(() => {
